...
public class PIDController implements Sendable, AutoCloseable {
	 ...
	 * @param periodSinceLastCall The time since calculate was last called (seconds)
	 */
	public double calculate(double measurement, double setpoint, double secondsSinceLastCalculate) {
	...
	public double calculate(double measurement, double secondsSinceLastCalculate) {
		if(secondsSinceLastCalculate == 0) {
			ArachneLogger.getInstance().warn("Called calculate with a time delta of 0");
		}
		else {
			m_prevError = m_positionError;
			m_positionError = getContinuousError(m_setpoint - measurement);
			m_velocityError = (m_positionError - m_prevError) / secondsSinceLastCalculate;
	
			if (m_Ki != 0) {
				m_totalError = MathUtil.clamp(
						m_totalError + m_positionError * secondsSinceLastCalculate,
