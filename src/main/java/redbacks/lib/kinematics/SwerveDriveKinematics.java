...
public class SwerveDriveKinematics {
	...
	private Translation2d prevCenterOfRotation = new Translation2d();
	...
	public SwerveDriveKinematics(Translation2d... wheelsMeters) {
		...
		this.numModules = wheelsMeters.length;
		this.modules = Arrays.copyOf(wheelsMeters, numModules);
		this.inverseKinematics = new SimpleMatrix(numModules * 2, 3);

		for(int i = 0; i < numModules; i++) {
			this.inverseKinematics.setRow(i * 2 + 0, 0, /* Start Data */ 1, 0, -modules[i].getY());
			this. inverseKinematics.setRow(i * 2 + 1, 0, /* Start Data */ 0, 1, +modules[i].getX());
		}
		
		this.forwardKinematics = this.inverseKinematics.pseudoInverse();
	}
	...
	public SwerveModuleState[] toSwerveModuleStates(ChassisSpeeds chassisSpeeds,
		...
		if(!centerOfRotationMeters.equals(prevCenterOfRotation)) {
			for(int i = 0; i < numModules; i++) {
				this.inverseKinematics.setRow(
					i * 2 + 0, 0, /* Start Data */ 1, 0,
					-modules[i].getY() + centerOfRotationMeters.getY()
				);
				this.inverseKinematics.setRow(i * 2 + 1, 0, 
					/* Start Data */ 
					0, 1,
					+modules[i].getX() - centerOfRotationMeters.getX()
				);
			}
			prevCenterOfRotation = centerOfRotationMeters;
		}
